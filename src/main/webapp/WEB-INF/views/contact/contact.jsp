...
<body class="content-body">
	...
	<form:form class="form-cadastro" modelAttribute="emailTemplate"
		action="${s:mvcUrl('CC#sendEmail').build() }" method="POST">
		<h1>Nos envie uma mensagem</h1>
		<h3>Preencha os campos abaixo</h3>
		<div class="form-group">
			<label for="email">Email</label>
			<form:input path="email" cssClass="form-control" />
			<form:errors path="email" cssClass="alert-danger" id="email" />
		</div>
		<div class="form-group">
			<label for="title">Titulo da Mensagem</label>
			<form:input path="subject" cssClass="form-control" />
			<form:errors path="subject" cssClass="alert-danger" id="title" />
		</div>
		<div class="form-group">
			<label for="text">Mensagem</label>
			<form:textarea path="text" cssClass="form-control" />
			<form:errors path="text" cssClass="alert-danger" id="text" />
		</div>
		<button type="submit" class="btn btn-outline-segundary my-2 my-sm-0">Enviar</button>
	</form:form>
